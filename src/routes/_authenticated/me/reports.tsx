  ...
  const submit = async () => {
    ...
    try {
      ...
      for (const file of photos) {
        ...
        const path = `${user.id}/${await contentHashName(optimized.blob, optimized.ext)}`;
        const { error: upErr } = await supabase.storage.from("daily-reports").upload(path, optimized.blob, { contentType: optimized.contentType, upsert: true });
